...
describe('Client CRUD tests', function() {
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/clients')
					...
					.end(function(clientSaveErr, clientSaveRes) {
						...
						agent.put('/clients/' + clientSaveRes.body._id)
							.send(client)
							.expect(200)
	...
	it('should be able to get a single Client if not signed in', function(done) {
		...
		// Save the Client
		clientObj.save(function() {
			request(app).get('/clients/' + clientObj._id)
				.end(function(req, res) {
					// Set assertion
			...
			.end(function(signinErr, signinRes) {
				...
				agent.post('/clients')
					...
					.end(function(clientSaveErr, clientSaveRes) {
						...
						agent.delete('/clients/' + clientSaveRes.body._id)
							.send(client)
							.expect(200)
	...
	it('should not be able to delete Client instance if not signed in', function(done) {
		...
		clientObj.save(function() {
			// Try deleting Client
			request(app).delete('/clients/' + clientObj._id)
			.expect(401)
			.end(function(clientDeleteErr, clientDeleteRes) {
				// Set message assertion
				(clientDeleteRes.body.message).should.match('User is not logged in');
